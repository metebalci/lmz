...
Uses GLOBAL:AREF, so will pop fixnums out of strings."
  (WITHOUT-INTERRUPTS
    (LET* ((INDEX (1- (FILL-POINTER ARRAY)))	;1- because fill-pointer is # active elements
	   ;; LMZ: (FUNCTION ARRAY-TYPES), as SYS; QRAND:1512 does it; SYMBOL-FUNCTION is wrong here.
...
;;; LMZ: MacLisp's STATUS and SSTATUS, which read and changed this list, are
...
;;; LMZ: the rest of the function-spec machinery, the part that need not be in
;;; the cold load; SYS: SYS; QRAND has the rest.  MIT was moving both to
;;; SYS: SYS; FSPEC LISP and stopped, leaving that file one block comment; LMZ
  ...
  (COND ((AND (EQ DOC-TYPE 'VALUE)
	...
	((COMPILED-FUNCTION-P SYMBOL)
	 (IF (ASSQ 'COMBINED-METHOD-DERIVATION (DEBUGGING-INFO SYMBOL))
	   ...
	   (OR (CADR (ASSQ 'DOCUMENTATION (DEBUGGING-INFO SYMBOL)))
	       (CADR (ASSQ :DOCUMENTATION (DEBUGGING-INFO SYMBOL))))))))
;;; LMZ: FUNCTION-DOCUMENTATION is gone: its DEFF alias to DOCUMENTATION was
