...
(DEFUN ARRAY-TYPE-NULL-ELEMENT (TYPE)
  (CDR (ASSQ (ARRAY-CANONICALIZE-TYPE TYPE)
	     '((ART-ERROR . NIL)
	       ...
	       (ART-FAT-STRING . #/ )
	       (ART-COMPLEX . 0)
	       ;; LMZ: the two complex float types have no entry.  Their null element
	       ...
	       ;; before the reader can make a complex number.  Nothing in LMZ calls
...
Blame Common Lisp for wanting this unrelated alternate meaning."
  (IF FORM
	...
	(VALUES-LIST VALUES))
    ;; LMZ: the CADR's clock only; the Lambda's clause is gone.
    (WITHOUT-INTERRUPTS
...
;;; LMZ: this is where the function-spec machinery lives.  MIT had begun to move
;;; it to SYS: SYS; FSPEC LISP and stopped, leaving that file one block comment
;;; and these definitions the live ones; LMZ deleted the empty file (#11), so
...
;;; LMZ: these too were being moved to SYS: SYS; FSPEC LISP, which LMZ deleted
...
(DEFUN FUNCTION-SPEC-DEFAULT-HANDLER (FUNCTION FUNCTION-SPEC &OPTIONAL ARG1 ARG2)
  ...
  (CASE FUNCTION
    ...
    (DWIMIFY NIL)
    ;; LMZ: the clause had a second form, ARG2, after the IF, so it always
