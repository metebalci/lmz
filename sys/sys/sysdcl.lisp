...
(DEFSYSTEM SYSTEM
  ...
  (:COMPONENT-SYSTEMS FONTS		;Before SYSTEM-INTERNALS because things use fonts
		      ...
		      COLOR
		      EH
;;;LMZ		      PRESS		;not carried: a Xerox Dover printer
		      MATH
		      HACKS
		      METER
		      SRCCOM
...
;;; LMZ: the PRESS system is not carried.  It printed to a Xerox Dover over
...
;;; still name the PRESS package; see metebalci/lmz#14.
;(DEFSYSTEM PRESS
;  (:PACKAGE PRESS)
;  (:PATHNAME-DEFAULT "SYS: IO1;")
...
(DEFSYSTEM CADR
  ...
  ;; LMZ: not patchable.  LMZ makes releases, never patches; only System keeps a patch directory, for its release number.
  ;; LMZ: :INITIAL-STATUS goes too; it signals an error on a system that is not patchable.
  (:NOT-IN-DISK-LABEL)
  (:PACKAGE CADR)
...
(DEFSYSTEM ZMAIL
  ...
  ;; LMZ: ZMAIL; PARSE, a parser generator nothing used, is deleted, and its commented-out module and load steps with it.
...
;;; LMZ: :MAGTAPE dropped from this list; SYS: TAPE; is not carried, and
...
;;; LMZ: the LMFILE file computer is not carried; SYS: FILE2; holds only
;;; PATHNM, which FILE-SYSTEM loads for LMFILE pathnames.  Its systems have no
;;; definition file left to point at.
