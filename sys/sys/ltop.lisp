...
;;; LMZ: the run-light and TV sync setup, lifted out of LISP-REINITIALIZE so
;;; that the screen code can ask for it by name.  SET-UP-SCAN-LINE-TABLE
...
;;; LMZ: this block is boot work --- unloading microcompiled definitions,
...
; (SETQ *RSET T)				;In case any MACLISP programs look at it
  ...
  ;; LMZ: no longer wrapped in SELECT-PROCESSOR; LMZ runs only on a CADR.
  (AND (FBOUNDP 'CADR:CLEAR-UNIBUS-MAP)	;clear valid bits on unibus map.
       (CADR:CLEAR-UNIBUS-MAP))		; and necessary if sharing Unibus with PDP11.
						...
						; LMZ: the ETHERNET code this ordering used to
...
; *READ//PRINT-BASE* = ")
  ...
  ;; LMZ: in a cold load, run the server's script if it has one, so that a
...
; (SETQ AREA-FOR-PROPERTY-LISTS WORKING-STORAGE-AREA)	;Because will be recopied at end
  ...
  (LET (TV:MORE-PROCESSING-GLOBAL-ENABLE)
    ...
    ;; LMZ: a supplied list (including NIL) lets COLDRUN choose the extra
