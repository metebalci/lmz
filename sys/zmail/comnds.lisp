...
(DEFUN GET-ZMAIL-BUFFER-ALISTS (&OPTIONAL OTHERS-TOO &AUX MFL TMFL MFL-PATHNAMES)
  ...
  (AND OTHERS-TOO
	 ...
	 (OR (MEMBER-EQUAL (SEND FILE-NAME :NEW-VERSION :NEWEST) MFL-PATHNAMES)
...
;; LMZ: the :VMS surface-type clause went with VMS support, and the :ITS
;; one with ITS support; LMZ never talks to either file server.
  ...
  (MULTIPLE-VALUE-BIND (MAIL-FILE-BUFFERS TEMP-ZMAIL-BUFFERS)
	...
	((NULL LIST)
	 (OR FIRST-P (SEND *TYPEOUT-WINDOW* :TYO #/CR)))
      (SETQ NAME (CAR LIST))
      (COND ((NOT (ASSOC-EQUAL NAME MAIL-FILE-BUFFERS))
	     (COND (FIRST-P
