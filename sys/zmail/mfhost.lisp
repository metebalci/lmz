...
;; LMZ: this used to also test for an :ITS host, appending its //G
;; switch; ITS support is gone, and no host is ever of that type now.
...
;; LMZ: the FS:ITS-PATHNAME-MIXIN methods that offered and detected
;; ITS-format mail files (:MAIL-FILE-FORMAT-COMPUTER and friends) and
;; SI:HOST-ITS-MIXIN's :GMSGS-PATHNAME went with ITS support. The mail
;; format itself, ITS-MAIL-FILE-MIXIN below, stays: RMAIL-FILE-BUFFER
;; and BABYL-MAIL-FILE-BUFFER still use it.
...
(DEFUN NAME-FROM-MENU-VALUE (VALUE ITEM-LIST)
  (DOLIST (ELEM ITEM-LIST)
    (AND (EQ (TV:MENU-EXECUTE-NO-SIDE-EFFECTS ELEM) VALUE)
	 (RETURN (CAR ELEM)))))

;; LMZ: Tenex and TOPS-20 mail-file support (TENEX-MAIL-FILE-MIXIN,
;; TENEX-MAIL-FILE-BUFFER, TENEX-INBOX-BUFFER, and the
;; FS:TENEX-FAMILY-PATHNAME-MIXIN, FS:TOPS20-PATHNAME-MIXIN,
;; FS:TENEX-PATHNAME-MIXIN and SI:HOST-TOPS20-MIXIN methods that offered
;; and detected it) removed with TOPS-20 and Tenex support; unlike
;; ITS-MAIL-FILE-MIXIN, nothing else used this mail format.
...
(DEFMETHOD (UNIX-MAIL-FILE-BUFFER :UPDATE-MSG-OPTIONS-IN-FILE) (MSG)
  (LET* ((-STATUS- (ASSURE-MSG-PARSED MSG))
    ...
    (OR (LOOP FOR X IN FROM AND Y IN OLD-FROM
  ...
  (LET ((START-BP (MSG-START-BP MSG)))
    (LET ((LINE (BP-LINE START-BP)))
      (COND ((STRING-EQUAL-START LINE *UNIX-FROM-MARKER*)
	     (SETQ UNIX-HEADER (PARSE-UNIX-FROM-HEADER LINE))
  ...
  (LET ((START (STRING-LENGTH *UNIX-FROM-MARKER*))
	...
	(())
      ...
      (AND ( NEXT-END (+ END 4))
	   (DOLIST (DAYLIST TIME:*DAYS-OF-THE-WEEK*)
	     (IF (STRING-EQUAL (CAR DAYLIST) WORD 0 0 (LENGTH WORD))
		 (RETURN T)))
	   (RETURN))
    ...
    (LIST (PARSE-ADDRESSES LINE START END)
	  (AND END
		   ...
		   (TIME:PARSE-UNIVERSAL-TIME LINE (+ END 1))
...
(DEFMETHOD (UNIX-INBOX-BUFFER :BEFORE :PARSE-MSG) (MSG -STATUS-)
  ...
  (PUTPROP -STATUS- T 'UNSEEN))

;; LMZ: VMS mail-file support (VMS-MAIL-FILE-MIXIN, VMS-MAIL-FILE-BUFFER,
;; VMS-INBOX-BUFFER, and the FS:VMS-PATHNAME-MIXIN methods that offered
;; and detected it) removed with VMS support; unlike ITS-MAIL-FILE-MIXIN,
;; nothing else used this mail format.
