...
;;; LMZ: restored from the tape content (System 99.32), which works the
...
;;; LMZ: NO-BARF makes a refused open return NIL instead of breaking, so that
...
;;; LMZ: the two OTHERWISE clauses below print the operation with ~S.  They
...
(DEFUN MINI-BINARY-STREAM (OP &OPTIONAL ARG1 ARG2)
  (CASE OP
    ...
    ;; LMZ: answer :SEND-IF-HANDLES instead of barfing.  It means "send this
...
(DEFUN MINI-ASCII-STREAM (OP &OPTIONAL ARG1 ARG2)
  (CASE OP
    ...
    ;; LMZ: see the note on the binary stream above.
    (:SEND-IF-HANDLES
...
;;; LMZ: a cold load runs unattended.  It asks the file server for
...
;;; LMZ: COLDRUN is generated site policy, not system source.  MINI has no
...
;;; LMZ: send one line for the server to log.  MINI has no way to send data,
...
(DEFUN MINI-REPORT (MESSAGE)
  ;; LMZ: QLD hands the board to the full NCP.  Reinitializing MINI after
...
;;; LMZ: after QLD, send the same report using an ordinary NCP connection.
...
;;; LMZ: MINI has one stream and one packet buffer.  Read through EOF before
