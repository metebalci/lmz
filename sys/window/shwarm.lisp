...
;;; LMZ: these three set the CADR's TV control register only; the Lambda's
...
;;; LMZ: the CADR's screen; the Lambda's sizes are gone.
...
This is useful if your monitor is adjusted so that some of it cannot be seen."
  ;; LMZ: the CADR's limits and default; the Lambda's are gone.
  (IF ARG
	...
	(LOCK-SHEET (WHO-LINE-SCREEN)
	  (WITHOUT-INTERRUPTS
	    (LET ((MS MOUSE-SHEET) (SW SELECTED-WINDOW))
	      ...
	      ;; LMZ: no longer wrapped in IF-IN-CADR; LMZ runs only on a CADR.
	      (SI:SETUP-CPT
...

;;;; Run lights
;;; LMZ: the Lambda's scan line table is deleted, and with it SET-SCREEN-WIDTH,
...
(defun set-up-scan-line-table ()
  ;; LMZ: on a CADR the run-light locations come from SYS; LTOP, which knows
