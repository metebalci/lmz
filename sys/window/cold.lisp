...
;;; LMZ: only the CADR's code is left.  SI:PROCESSOR-TYPE-CODE, which the
...
;;; LMZ: SET-MOUSE-MODE and VIRTUAL-UNIBUS-ADDRESS keep the CADR's branch only.
...
(DEFUN SETUP-CPT (&OPTIONAL (SYNC-PROG CPT-SYNC2)
			    (TV-ADR NIL)
			    (FORCE-P NIL))
  ;; LMZ: no longer wrapped in a choice on the processor; LMZ runs only on a CADR.
     FORCE-P
...
(DEFUN COLD-LOAD-STREAM-INIT-PLIST-GENERATOR NIL
  ;; LMZ: the CADR's size; the Lambda's is gone.
  `(:WIDTH 1400
    :HEIGHT 1600
