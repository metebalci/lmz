	  ...
	  (DOLIST (ELEM CONNECTIONS)
		...
		(CASE (STATE CONN)
		  ...
		  (ANSWERED-STATE
		    ...
		    ;; LMZ: with no host, HOSTAT waited for an answer that cannot come.
...
;; LMZ: PARSE-TWENEX-FINGER, for :TOPS-20 hosts, removed with TOPS-20
;; support; LMZ never talks to a TOPS-20 host. (PARSE-TENEX-FINGER, below,
...
;; LMZ: PARSE-ITS-FINGER removed with ITS support; LMZ never talks to an
;; ITS host.
...
(DEFUNP EVAL-SERVER-FUNCTION (&AUX CONN)
  ...
  (CATCH-ERROR
    (WITH-OPEN-STREAM (STREAM (MAKE-STREAM CONN :ASCII-TRANSLATION T))
      ...
      ;; LMZ: peek rather than read and :UNTYI, as the TELNET server does (#2): a
  ...
  (LET ((LOSE (DISALLOW-CONNECTION? "TELNET" CONN
	...
	(LET ((UNTYI-CHAR NIL))
	  ...
	  (WITH-OPEN-STREAM (STREAM (MAKE-STREAM CONN :ASCII-TRANSLATION T))
	    ...
	    (SEND STREAM :FORCE-OUTPUT)
	    ;; Flush any number of telnet negotiations.  (We only understand the simplest kind).
	    ;; LMZ: peek at the next byte instead of reading it and sending :UNTYI (#2).  The
...
(DEFUN GENERATE-HOST-TABLE (&OPTIONAL INPUT-FILE)
  ;; LMZ: the host table is a site file, and lives in SYS: SITE;.
