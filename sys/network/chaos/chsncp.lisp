...
;;; LMZ: 256 subnets rather than 96, which is every subnet an address can
...
;;; traps as soon as the network starts.  LMZ's own site is on subnet 376,
...
(DEFUN RESET-ROUTING-TABLE ()
  ...
  ;; LMZ: always the Chaos board; the Lambda reached its subnet over Ethernet.
...
(DEFUN SETUP-MY-ADDRESS ()
  ;; LMZ: the address comes from the Chaos board; the Lambda took it from the disk label.
...
;;; LMZ: these three drive the Chaos board; the Lambda, which had none, did nothing.
...
(DEFUN TRANSMIT-INT-PKT (INT-PKT &OPTIONAL (HOST (PKT-DEST-ADDRESS INT-PKT))
  ...
  ;; LMZ: the Lambda's :ETHERNET route, which shared its Ethernet board with
