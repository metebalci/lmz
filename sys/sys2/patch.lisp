...
;;; LMZ: LOAD-PATCHES and LOAD-AND-SAVE-PATCHES, and RESERVE-PATCH and the rest
;;; of what the editor's patch commands used, are deleted.  LMZ makes releases,
  ...
  (WITH-OUTPUT-TO-STRING (S)
    ...
    (DOLIST (SYS PATCH-SYSTEMS-LIST)
      (COND ((NOT (AND BRIEF-P (SYSTEM-SHOULD-NOT-APPEAR-IN-DISK-LABEL (PATCH-NAME SYS))))
	     ...
	     ;; LMZ: the major version only.  LMZ makes no patches, so every
...
The microcode version number and some other suitable information is also included."
  ...
  (DO ((SYS PATCH-SYSTEMS-LIST (CDR SYS))
    ...
    (DOTIMES (I (- MAX (ARRAY-ACTIVE-LENGTH (CAR NAM))))
      (WRITE-CHAR #/SPACE S))
    ;; LMZ: the major version only, as in SYSTEM-VERSION-INFO above.
