...
(DEFUN DISK-RUN (RQB UNIT ADDRESS SECTORS-PER-TRACK HEADS-PER-CYLINDER CMD CMD-NAME
 ...
 Type ~:C to retry."
		       ...
		       #/RESUME)
	       (GO FULL-RETRY))
	      ;; LMZ: the test that this is not a Lambda is gone; on a CADR it was always true.
	      ((OR ( FINAL-CYLINDER
		     (LDB #o0014 (AREF RQB %DISK-RQ-FINAL-UNIT-CYLINDER)))
		   ( FINAL-SURFACE
		     (LDB #o1010 (AREF RQB %DISK-RQ-FINAL-SURFACE-SECTOR)))
...
;;; LMZ: MT (magtape) was a third case here; FS:MAKE-BAND-MAGTAPE-HANDLER never
...
(DEFUN SYS-COM-PAGE-NUMBER (16B-BUFFER INDEX)
  (LSH
    (LOGAND
      (1- 1_24.)				;LMZ: the CADR's mask; the Lambda's is gone.
...
or /"CC/" which refers to the machine being debugged by this one."
  (SETQ FILENAME (IF (NUMBERP FILENAME)
		     ;; LMZ: UCADR only; there is no ULAMBDA.
    ...
    (PROGN
      ...
      (LET ((COMMENT (AND DESC-LOC
			  ...
			  (GET-DISK-STRING RQB
					   (+ DESC-LOC 3)
					   (* 4 (- (GET-DISK-FIXNUM RQB #o201) 3))))))
	;; LMZ: only UCADR comments; the Lambda's ULAMBDA ones are gone.
	(AND COMMENT
...
Use SI:RECEIVE-BAND or SI:TRANSMIT-BAND for that."
  ...
  (UNWIND-PROTECT
	...
	;; LMZ: this used to skip ahead for a magtape FROM-UNIT, which needed a
