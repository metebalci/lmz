...
Returns T if the band was set as specified, NIL if not
 (probably because user said no to a query)."
  ...
  (UNWIND-PROTECT
   (PROG ((UCODE-NAME "MCR"))			;LMZ: the Lambda's LMC is gone.
...
(DEFUN FIND-MICROCODE-PARTITION (RQB MICROCODE-VERSION
  ...
  ;; LMZ: the CADR's names only; the Lambda's ULAMBDA and LMC are gone.
...
or /"CC/" which refers to the machine being debugged by this one."
  ...
  (UNWIND-PROTECT
     (PROGN (WITHOUT-INTERRUPTS
	    ...
	    ;; LMZ: editing an existing label showed a fresh one, the label never
	    ;; having been read.
	    (IF (NULL INIT-P)
