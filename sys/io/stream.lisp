...
;;; LMZ: take :READ-CHAR's arguments, EOF-ERROR-P and EOF-VALUE, as the string stream
...
(DEFUN TYI-FROM-ASCII-STREAM (ASCII-STREAM &AUX CH)
  (CASE (SETQ CH (SEND ASCII-STREAM :TYI))
    ...
    (#o12 #/LINE)
    (#o14 #/FF)
    ;; LMZ: push a byte that is not LF back with SELF's :UNTYI (#2).  ASCII-STREAM is the
...
;;; LMZ: an ASCII stream rings its terminal's bell with BEL, ASCII 7, which the translation
...
;;; LMZ: compare the character's code, so that a character object translates as its
