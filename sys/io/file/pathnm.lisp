...
;;; LMZ: this used to also point at TOPS20-PATHNAME-MIXIN, gone with TOPS-20
;;; support.
...
;; LMZ: the :VMS surface-type clauses below went with VMS support, and the
;; (:TOPS-20 :TENEX) ones with TOPS-20 and Tenex support; LMZ never talks
...
(DEFUN DEFAULT-HOST (DEFAULTS &AUX ELEM)
  ...
  ;; LMZ: (NIL) and ((NIL)) are defaults lists holding no default, which is
  ...
  (COND ((AND DEFAULTS (ATOM DEFAULTS))
	...
	(T
	 ...
	 (OR (CDR ELEM)
	     ;; LMZ: before login USER-LOGIN-MACHINE is NIL, and this made a
...
specified by PATHNAME."
  ...
  (when (or (null (pathname-name pathname))
	    always-merge-name)
    ;; LMZ: this clause computes the name, and set NEW-VERSION by mistake.
