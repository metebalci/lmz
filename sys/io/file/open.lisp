...
;; LMZ: :LMFILE is gone from this test with the LMFILE file computer, and
...
(defun pathname-completion-list (defaults string type version &rest options
  ...
  (let ((name (send pathname :name)))
	...
	(if position (setq name (substring name 0 position))))
      ...
      ;; LMZ: the name is used as it stands.  Parsing it and taking :NAME back out
...
;;; LMZ: the Patch-File attribute no longer binds this, since LMZ loads no
