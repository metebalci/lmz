# What LMZ takes from upstream, and what it refuses

LMZ forks LM-3's **System 100**, which is the MIT tape content of release 99.32
...
130, if it is a style sweep, or if it is release bookkeeping for a release LMZ
does not have.
...
since LMZ inherits it. 101 files differ between release 99.32 and System 100,
...
are the part LMZ should care about, because each is a question left open rather
than answered:

| File | What the note says |
|---|---|
| `sys/fspec.lisp` ([#11](https://github.com/metebalci/lmz/issues/11)) | **the whole file disabled** with `#\|`, because loading `SYS2; DEFSEL QFASL` during `SI:QLD` barfed in `INTERNAL-FUNCTION-SPEC-HANDLER`. Settled for 1001: the file is deleted and the machinery stays in `qrand` and `qmisc`. |
| `sys/qmisc.lisp` ([#11](https://github.com/metebalci/lmz/issues/11)) | the functions above "should be moved to `SYS: SYS; FSPEC`, but due to issues (see FSPEC) hasn't been done just yet". Settled with the row above. |
| `eh/eh.lisp`, `eh/ehc.lisp` ([#16](https://github.com/metebalci/lmz/issues/16)) | "No way known to do LOCF on SG-PLIST" |
| `sys2/macarr.lisp` ([#15](https://github.com/metebalci/lmz/issues/15)) | "This version of STORE from NIL is not MACLISP compatible", the code disabled |
| `sys/qrand.lisp` ([#13](https://github.com/metebalci/lmz/issues/13)) | "MAKE-COLD doesn't support complex types" |
| `cold/mini.lisp` | the file server's address hard-coded to OZ's, since restored to the tape's version (`bdf0d71`) |
| `network/host.lisp`, `sys/eval.lisp`, `io/file/logical.lisp` ([#12](https://github.com/metebalci/lmz/issues/12)) | "are these still used?" |
| `sys/sysdcl.lisp` ([#14](https://github.com/metebalci/lmz/issues/14)) | "How is PRESS-FONTS; FONTS WIDTHS generated?" |

Those are LMZ's inheritance and its first list of real work: a release that
...
before the change is read against LMZ's own source: a commit message is a claim,
not evidence.

## Changes LMZ has made where upstream made one too

These were made on their own merits. Neither is a cherry-pick.

| Change | LMZ commit | Note |
|---|---|---|
| `network/chaos/chsncp.lisp`: routing table 96 → 256 subnets | `e4cc865` | A Chaos subnet is the high byte of a 16-bit address, so 256 is the whole range and 96 leaves two thirds unreachable. LMZ's site is on subnet 376, muir-fpga's boards likewise, so nothing here could run as it stood. LM-3 changed the same constant; the change is obvious enough that both arriving at it means nothing. |
| `cold/mini.lisp`: work out the file server's address instead of naming it | `bdf0d71` | **A restoration, not an import.** Release 99.32 --- the tape content --- computes the address from the host of the file being compiled. System 100 replaced that with OZ's own `#o3060` on subnet 6, with a `;;;---!!!` note admitting it. LMZ puts MIT's version back. LM-3 also reverted to it, later. |
...
`qcom.lisp` was taken in LMZ's own form: the CADR's value alone, since this
...
one FORMAT inside another's arguments) is a real fault in a tool LMZ has decided
never to use, since LMZ makes only major releases and no patches. Deferred: `c1f37f7`
...
`5246061`, the MINI server, is worth a second look: ozd serves MINI for LMZ, so
