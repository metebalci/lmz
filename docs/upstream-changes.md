...
|---|---|
| `sys/fspec.lisp` ([#11](https://github.com/metebalci/lmz/issues/11)) | **the whole file disabled** with `#\|`, because loading `SYS2; DEFSEL QFASL` during `SI:QLD` barfed in `INTERNAL-FUNCTION-SPEC-HANDLER` |
| `sys/qmisc.lisp` ([#11](https://github.com/metebalci/lmz/issues/11)) | the functions above "should be moved to `SYS: SYS; FSPEC`, but due to issues (see FSPEC) hasn't been done just yet" |
