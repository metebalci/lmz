...
What LMZ System 1001 changes from System 1000. Its theme is a deep cleanup:
what LMZ will never use is removed, not commented out, and git keeps the
...
- **Firmware tools for hardware LMZ does not have:** the keyboard's 8748
...
- **Patches.** LMZ never makes or loads a patch, so the machinery for them
...
- **Lambda and Explorer support** (#17). LMZ runs on a CADR, or on muir and
  ...
  - The 28 `DEFMIC`s of instructions the CADR's microcode never implemented
    ...
    and the opcodes are free for LMZ's own.
...
- **Host operating systems LMZ will never talk to:** it talks to exactly
  ...
  - **TOPS-20, Tenex and Twenex:** `TENEX-FAMILY-PATHNAME-MIXIN` (the
    ...
    which LMZ keeps); the dead `(:TOPS-20 :TENEX)` surface-type clauses
  ...
  - **Not touched, reported instead:** `cold/export.lisp` still exports
    ...
    LMZ's own local file system, out of scope for this cleanup; both are
...
- **The demos that need hardware LMZ does not have** (the user, 2026-09-18):
...
- **The site is LMZ, not Z54,** and its hosts are numbered from the subnet's
