...
What LMZ release 1000 changes from its base, LM-3's **System 100** --- the tape
...
- **`tape/`**, and the MagTape code that lived in `file/`: `mtdefs`, `mtstr`,
  `mtaux`, `odump`. No machine LMZ targets has a tape drive.
...
- **Every patch file and every patch directory except System's own.** LMZ makes
...
- **The herald always says LMZ** (`io/disk.lisp`, `PRINT-HERALD`). It used to
  choose "MIT" or "LMI" by the site name.
...
- **The LMZ site** in `site/`. `sys.translations` doubles its slashes and names
