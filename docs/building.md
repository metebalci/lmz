...
- **A running band** to compile with, such as LMZ 1000.
...
The readtable compiler is in no system (`sys/sysdcl.lisp` does not name `IO; RTC`), and an LMZ 1000 band does not have it loaded: `(fboundp 'si:rtc-file)` is `NIL`. So compile and load it first:
...
`sys/ucadr/`. Verified on 2026-09-17 on an LMZ 1000 band: everything it holds
