...
**Status:** the procedure below was verified end to end on 2026-09-16, against a tree based on LM-3's System 304. LMZ has since re-based on System 100, and the same build has not yet been run on it. The stages, the forms and the traps are the system's own and do not depend on the base; the line citations and the file list do, and are being checked as the build is repeated. The three source faults below are present in System 100 at the same places. LMZ compiled its whole tree, built a cold load, booted it, loaded the world through MINI and the file protocol, and saved a band that boots and answers.
