...
**Lisp Machine Z is a continuation of MIT's Lisp Machine project.**
...
LMZ forks there and continues.

LMZ deliberately does **not** take LM-3's later releases. Between System 100
...
material that is not MIT's left out, and a greeting that says LMZ. The number
...
- **The name and the number.** `PRINT-HERALD` always says LMZ
...
- **`sys/sys2/prodef.lisp`:** `SCHEDULER-STACK-GROUP` is declared `NIL` instead
  ...
  with nothing printed at all. ([#6](https://github.com/metebalci/lmz/issues/6))
- **`sys/cold/mini.lisp`:** the "Unknown stream operation" error now names the
  ...
  was the one fact missing. ([#7](https://github.com/metebalci/lmz/issues/7))
- **`sys/cold/mini.lisp`:** both MINI streams now answer `:SEND-IF-HANDLES`,
  ...
  ([#7](https://github.com/metebalci/lmz/issues/7))
...
([#9](https://github.com/metebalci/lmz/issues/9)), is worked around in this
...
LMZ is the system. Three projects around it carry the machine it runs on:
...
**LMZ has no patches, only releases.** Every change reaches a band through a
...
track of the release number, but LMZ writes no patch files and loads none.
...
The system sources are MIT's, by way of LM-3. LMZ's own changes, the builds and
the documents here are made by [Claude Code](https://claude.com/claude-code),
on Anthropic's Opus and Fable models.

## Copying

LMZ is licensed under the **GNU Affero General Public License, version 3 or
later**; see [`LICENSE`](LICENSE). LM-3 released System 100 under those terms,
and LMZ continues under them.
